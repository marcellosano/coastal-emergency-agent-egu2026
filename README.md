...
Three things make this defensible. Every recommendation is **grounded in a citation to a specific plan paragraph**, not free-form opinion. Every brief carries an **audit trail** of which information sources the language model consulted. And the system has hard kill switches — a daily cap on automated briefings, a single environment variable that disables the language model instantly, and a stand-down rule that closes the incident when the surge falls back below 100 cm for two consecutive forecast ticks.
